...
function [s]=st_iso_epoch(tgps,ndec)
%% -*- texinfo -*-
...
%% @item @var{ndec} : seconds decimal number
%% @end itemize  
%% @item Output :
%% @itemize @minus
...
if ndec>0
	fmt=sprintf('%s%d%s%d%s','%04d-%02d-%02dT%02d:%02d:%0',ndec+3,'.',ndec,'f');
else
	fmt='%04d-%02d-%02dT%02d:%02d:%02d';
end

s=sprintf(fmt,tgps.yyyy,tgps.mon,tgps.dd,tgps.hh,tgps.min,tgps.sec);
