...
while(i<100)
    ...
    if RNX.VERSION>=3
        ...
        % Ionospheric correction
        if strcmp(header(i-1,61:69),   'ION ALPHA')
            for j=1:4
                RNX.GPSA(j)=str2num(header(i-1,3+(j-1)*12:3+(j)*12));
            end
        end
        if strcmp(header(i-1,61:68),   'ION BETA')
            for j=1:4
                RNX.GPSB(j)=str2num(header(i-1,3+(j-1)*12:3+(j)*12));
            end
        end
